...
public final class EversoloHttpEngine implements PlaybackEngine {
    ...
    // getSpectrum used to be read here. It is gone: on this unit the endpoint answers
    // {} for every source and getState reports isHasSpectrum=false, so there was never
    // anything behind it. The analyser now decodes the file itself - see FileSpectrum.
    // API_FINDINGS.md records exactly what was tried, so nobody repeats it.
